...
exports.createUser = catchAsync(async (req, res, next) => {
  ...
  const newUser =
    await db.query(`INSERT INTO "User" Values(DEFAULT, '${FName}', '${LName}', '${PhoneNumber}', '${image}',
  ...
  if (role === "Seller") {
    await db.query(
      `INSERT INTO Seller Values('${newUser["rows"][0]["id"]}','${NId}');`
    );
  }
  if (role === "Customer") {
    await db.query(
      `INSERT INTO Customer Values('${newUser["rows"][0]["id"]}','Normal');`
    );
  }
...
exports.protectForSeller = catchAsync(async (req, res, next) => {
  ...
  const isSeller = await db.query(
    `SELECT * FROM Seller WHERE id = ${freshUser["rows"][0]["id"]}`
  );
...
exports.protectForCustomer = catchAsync(async (req, res, next) => {
  ...
  const isCustomer = await db.query(
    `SELECT * FROM Customer WHERE id = ${freshUser["rows"][0]["id"]}`
  );
...
exports.protectForEmployee = catchAsync(async (req, res, next) => {
  ...
  // @ts-ignore
  const freshUser = await db.query(
    `SELECT * FROM employee WHERE id = ${decoded.id}`
  );
...
exports.protectFromCustomer = catchAsync(async (req, res, next) => {
  ...
  // 3) check if the user still exist
  // @ts-ignore
  let freshUser = await db.query(
    `SELECT * FROM customer WHERE id = ${decoded.id}`
  );
