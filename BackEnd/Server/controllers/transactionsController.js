...
exports.buy = catchAsync(async (req, res, next) => {
  ...
  const shipmentId = await db.query(
    "SELECT currval('transaction_shipmentid_seq') AS last_value;"
  );
  ...
  await db.query(
    `UPDATE "User" SET balance = ${
      balance["rows"][0]["balance"] - cost
    } WHERE id = ${customerId};`
  );
  for (const item of cart) {
    await db.query(
      `UPDATE product SET quantity = quantity-${item.Quantity} WHERE id = ${item.productId};`
    );
    const result = await db.query(
      `INSERT INTO transaction VALUES (default,${customerId},${item.productId},'${formattedDate}',false, false,${item.shippedvia}, ${item.Quantity},${shipmentId["rows"][0]["last_value"]} );`
    );
  }
...
exports.getTranscationsforSeller = catchAsync(async (req, res, next) => {
  ...
  const result =
    await db.query(`SELECT transaction.*, product.name, product.price, product.price
      FROM transaction
...
exports.getTranscationsforCustomer = catchAsync(async (req, res, next) => {
  const userId = req.user;
  const result =
    await db.query(`SELECT transaction.*, product.name, product.price
    FROM transaction
    JOIN product ON transaction.productid = product.id
