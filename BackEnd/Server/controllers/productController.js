...
exports.createProduct = catchAsync(async (req, res, next) => {
  const { Name, Image, PreRelease, Price, Description, Quantity, Category } =
    req.body;
  ...
  const result = await db.query(
    `SELECT * FROM product WHERE name = '${Name}' AND sellerId = ${sellerId};`
  );

  const minn =
    await db.query(`SELECT w.id , w.maxquantity , COALESCE(SUM(p.quantity), 0) AS current_capacity FROM warehouse w
  ...
  if (result["rowCount"] === 0) {
    const adding =
      await db.query(`INSERT INTO product VALUES(default,'${Image}','${Name}',${PreRelease},
...
exports.AddToWishList = catchAsync(async (req, res, next) => {
  ...
  const result = await db.query(
    `INSERT INTO wishList VALUES (${customerId} , ${productId} ,'${formattedDate}')`
  );
...
exports.deleteProduct = catchAsync(async (req, res, next) => {
  ...
  const result = await db.query(
    `UPDATE product SET quantity = -1 WHERE id = ${productId}`
  );
